...
def volume_bonus(vol, num_points):
...
def optimize(inside_points, model):
    ...
    for i in range(num_steps):
        ...
        bonus = volume_bonus(model.volume(), num_points)
...
def optimize_basin_hopping(inside_points, model):
    model.eval()
    ...
    def optimize_callback(x):
        set_model(model, x)
        jaccard_index = model(inside_points)
        bonus = volume_bonus(model.volume(), inside_points.shape[0])
        loss = (-jaccard_index - bonus).item()
        print(loss)
        return loss

    x0 = np.array([model.position.data[0].item(), model.position.data[1].item(), model.position.data[2].item(),
                   model.rotation.data[0].item(), model.rotation.data[1].item(), model.rotation.data[2].item(),
                   model.rotation.data[3].item(),
                   model.inverse_scale.data[0].item(), model.inverse_scale.data[1].item(),
                   model.inverse_scale.data[2].item()])

    ret = scipy.optimize.basinhopping(optimize_callback, x0, T=0.1)
    set_model(model, ret.x)
...
while len(connected_components) > 0 and i < max_num_fitted_models:
    ...
    best_model = argmin([BoxModel(points, lambda_)], partial(optimize_basin_hopping, points))
